...
CHECKER_ID = "check_asam_xosc_minsubset_start_triggers_not_allowed"
CHECKER_DESCRIPTION = "Input file must not contain any start triggers."
CHECKER_PRECONDITIONS = basic_preconditions.CHECKER_PRECONDITIONS
RULE_UID = "asam.net:xosc:1.3.0:minsubset.start_triggers_not_allowed"
...
def _no_triggers_except_storyboard_stop_trigger(
    ...
    issues = []
    for xml_stop_trigger in xml_tree.findall(".//StartTrigger"):
        logging.error(f"- Found disallowed StartTrigger")
        issue = {
            "description": f"Found disallowed StartTrigger",
...
def check_rule(checker_data: models.CheckerData) -> None:
    ...
    for issue in issues_vehicle_lacks_front_axle:
        issue_id = checker_data.result.register_issue(
            ...
            description="Issue flagging when input file contains disallowed StartTrigger",
            level=RULE_SEVERITY,
            rule_uid=RULE_UID,
        )
