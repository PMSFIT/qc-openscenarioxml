...
CHECKER_ID = "check_asam_xosc_minsubset_entity_selection_not_allowed"
CHECKER_DESCRIPTION = "Entity selections are not allowed."
CHECKER_PRECONDITIONS = basic_preconditions.CHECKER_PRECONDITIONS
RULE_UID = "asam.net:xosc:1.3.0:minsubset.entity_selection_not_allowed"
...
def check_rule(checker_data: models.CheckerData) -> None:
    ...
    issue_contains_entity_selections = _contains_entity_selections(
        xml_tree=checker_data.input_file_xml_root
    )

    for issue in issue_contains_entity_selections:
        issue_id = checker_data.result.register_issue(
            checker_bundle_name=constants.BUNDLE_NAME,
            checker_id=CHECKER_ID,
            description="Issue flagging when input file contains EntitySelections",
            level=RULE_SEVERITY,
            rule_uid=RULE_UID,
        )
